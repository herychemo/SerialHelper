...
public class RXTXCommDriver implements CommDriver {
    ...
    static {
        ...
        if (OS.contains("indows")||OS.contains("inux")) 
            System.load(new DriverHelper().getPath(OS));
        else 
        ...
        if (CandidateDeviceNames != null && ValidPortPrefixes != null) {
            for (i = 0; i < CandidateDeviceNames.length; i++) {
                for (p = 0; p < ValidPortPrefixes.length; p++) {
                  ...
                  //Multiples conexiones sin errores de lockeo fix :3	<
                    if(new File("/var/lock/LCK.."+C).exists())
                    	new File("/var/lock/LCK.."+C).delete();
                  //Multiples conexiones sin errores de lockeo fix :3 	>
        ...
        switch (PortType) {
            case CommPortIdentifier.PORT_SERIAL:
                ...
                if (osName.equals("Linux")) {
                    String[] Temp = {
                    	"ttyACM",	//Linux CDC devices fix :3
